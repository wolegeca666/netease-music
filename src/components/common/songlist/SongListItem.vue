<template>
  <div :class="{'hover': !animate}" style="perspective: 1000px;"
    ...
    <div class="song-item"
         :class="{'active': num === currentIndex, 'animate': animate, 'odd': isOdd(oddIndex)}"
      ...
      <div class="song-msg">
        <div class="song-name">
          <p>
            <span>{{song.name}}</span>
            <span v-if="music.alias.length!==0" style="color:rgba(0,0,0,0.4);">{{' (' + music.alias[0] + ')' }}</span>
          </p>
        </div>
        <div class="author">
          <p>{{ authorHandle(song) }}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      musicPlay() {
        ...
        this.$store.commit('changeId', this.song.id);
      }
    },
    computed: {
      music: function () {
